...
class GanymedeHandler(ZMQChannelsHandler):
    ...
    """
    # Log a message sent from Jupyter client. This only triggers on one direction of communication (i.e. sending),
    # while _on_zmq_reply triggers on both sending+receiving. We don't want to trigger on sending twice, which is
    # why we are only relying on _on_zmq_reply.
